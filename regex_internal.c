...
// SMR-陶志远-20241227-004 内存函数，代替原有系统函数
reg_mem_t g_reg_mem = {0};
...
build_wcs_buffer (re_string_t *pstr)
{
  ...
  for (byte_idx = pstr->valid_len; byte_idx < end_idx;)
      ...
      else if (__glibc_unlikely (mbclen == (size_t) -2))
	{
      ...
      /* Write paddings.  */
        for (remain_len = (Idx)(byte_idx + mbclen - 1); byte_idx < remain_len ;)
	...
	if (__glibc_likely (0 < mbclen && mbclen < (size_t) -2))
	  {
		...
		if (__glibc_likely (mbclen == mbcdlen))
		...
		else if (mbcdlen != (size_t) -1)
		  {
		    ...
		    if (byte_idx + mbcdlen > (size_t)pstr->bufs_len)
		      {
			pstr->cur_state = prev_st;
			break;
		      }
			...
			pstr->offsets[byte_idx + i]
			  = (Idx)(src_idx + (i < mbclen ? i : mbclen - 1));
		...
		size_t i;
		for (i = 0; i < mbclen; ++i)
		  pstr->offsets[byte_idx + i] = (Idx)(src_idx + i);
	      }
	    src_idx += (Idx)mbclen;
			  ...
			  if ((size_t)(raw + offset - p) <= mbclen
			      && mbclen < (size_t) -2)
			    {
			      memset (&pstr->cur_state, '\0',
				      sizeof (mbstate_t));
			      pstr->valid_len = (Idx)(mbclen - (raw + offset - p));
			      wc = wc2;
			    }
			  break;
			}
...
re_node_set_merge (re_node_set *dest, const re_node_set *src)
{
  ...
  if (src == NULL || src->nelem == 0)
    return REG_NOERROR;
    if (dest->alloc < /*2*/4 * src->nelem + dest->nelem)
    {
      Idx new_alloc = /*2*/4 * (src->nelem + dest->alloc);
		    ...
		    re_hashval_t hash)
{
  ...
  newstate = (re_dfastate_t *) re_calloc (sizeof (re_dfastate_t), 1);
