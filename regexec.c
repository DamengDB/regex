		  ...
		  Idx stop, bool ret_len)
{
  ...
  if (__glibc_unlikely (length1 < 0 || length2 < 0 || stop < 0 || len < length1))
    return -2;
	      ...
	      int regs_allocated)
{
  ...
  if (regs_allocated == REGS_UNALLOCATED)
    ...
    { /* Yes.  If we need more elements than were already
	 ...
	 leave it alone.  */
      if (__glibc_unlikely ((__re_size_t)need_regs > regs->num_regs))
	{
  ...
  else
    {
      ...
      /* This function may not be called with REGS_FIXED and nregs too big.  */
        assert (regs->num_regs >= (__re_size_t)nregs);
      rval = REGS_FIXED;
    }
		    ...
		    regmatch_t pmatch[], int eflags)
{
  ...
  extra_nmatch = (Idx)((nmatch > preg->re_nsub) ? nmatch - (preg->re_nsub + 1) : 0);
  nmatch -= extra_nmatch;
  ...
  {
      /* TRUE：贪婪模式（最长匹配）；FALSE：非贪婪模式（最小匹配） */
      fl_longest_match = preg->fl_longest_match;
  }

  err = re_string_allocate (&mctx.input, string, length, (Idx)(dfa->nodes_len + 1),
  ...
  if (nmatch > 0)
    {
      ...
      for (reg_idx = 1; reg_idx < (Idx)nmatch; ++reg_idx)
      ...
      if (!preg->no_sub && nmatch > 1)
	{
	 ...
	 from 0.  */
      for (reg_idx = 0; reg_idx < (Idx)nmatch; ++reg_idx)
      ...
      for (reg_idx = 0; reg_idx < extra_nmatch; ++reg_idx)
	{
      ...
      if (dfa->subexp_map)
	for (reg_idx = 0; reg_idx + 1 < (Idx)nmatch; reg_idx++)
      ...
      if (idx == pmatch[0].rm_eo && cur_node == mctx->last_node)
	{
	  ...
	  if (fs)
	      ...
	      if (reg_idx == nmatch)
		{
	      ...
	      cur_node = pop_fail_stack (fs, &idx, (Idx)nmatch, pmatch,
					 &eps_via_nodes);
	    }
	  else
	    {
      ...
      cur_node = proceed_next_node (mctx, (Idx)nmatch, pmatch, &idx, cur_node,
				    &eps_via_nodes, fs);
  ...
  for (sub_top_idx = 0; sub_top_idx < mctx->nsub_tops; ++sub_top_idx)
      ...
      for (; sl_str <= bkref_str_idx; ++sl_str)
	{
	  ...
	  if (sub_top->path == NULL)
	    {
	      sub_top->path = re_calloc (sizeof (state_array_t),
...
build_trtable (const re_dfa_t *dfa, re_dfastate_t *state)
{
  ...
  if (__glibc_unlikely (ndests <= 0))
      ...
      if (ndests == 0)
	{
	  state->trtable = (re_dfastate_t **)
	    re_calloc (sizeof (re_dfastate_t *), SBC_MAX);
  ...
  if (__glibc_unlikely (ndests_max < (size_t)ndests))
    goto out_free;
  }
  ...
  if (!__glibc_unlikely (need_word_trtable))
    {
	 ...
	 256-entry transition table.  */
      trtable = state->trtable =
	(re_dfastate_t **) re_calloc (sizeof (re_dfastate_t *), SBC_MAX);
  ...
  else
    {
	 ...
	 starting at trtable[SBC_MAX].  */
      trtable = state->word_trtable =
	(re_dfastate_t **) re_calloc (sizeof (re_dfastate_t *), 2 * SBC_MAX);
      if (__glibc_unlikely (trtable == NULL))
	goto out_free;
...
match_ctx_init (re_match_context_t *mctx, int eflags, Idx n)
{
  ...
  if (n > 0)
	...
	MAX (sizeof (struct re_backref_cache_entry),
      ...
      /* SIZE_MAX为无符号-1，不能向Idx(int)类型转换，否则值为负 */
      if (__glibc_unlikely (MIN (IDX_MAX, SIZE_MAX / max_object_size) < (size_t)n))
	return REG_ESPACE;
...
match_ctx_add_subtop (re_match_context_t *mctx, Idx node, Idx str_idx)
{
  ...
  mctx->sub_tops[mctx->nsub_tops] = re_calloc (1, sizeof (re_sub_match_top_t));
...
match_ctx_add_sublast (re_sub_match_top_t *subtop, Idx node, Idx str_idx)
{
  ...
  new_entry = re_calloc (1, sizeof (re_sub_match_last_t));
...
{
    g_reg_mem.mem_malloc  = mem_malloc_fun;
    g_reg_mem.mem_realloc = mem_realloc_fun;
    g_reg_mem.mem_free    = mem_free_fun;
    g_reg_mem.mem_calloc  = mem_calloc_fun;
}
